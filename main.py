...
def get_all_audit_leaders(workbook_path):
    """Collect all unique Audit Leaders across all sheets, focusing on the main data table not summary tables."""
...
def create_leader_workbook(input_path, output_path, leader_name):
    """Create a workbook with only data for the specified leader, keeping all sheets.
    Also highlight tabs red if they contain DNC results for this leader, otherwise green."""
    ...
    try:
        ...
        for sheet_name in new_wb.sheetnames:
            ...
            if sheet_name in sheets_with_dnc:
                ...
                print(f"  🔴 Tab for sheet {sheet_name} colored RED (has DNC)")
            else:
                # Green tab for sheets without DNC
                sheet.sheet_properties.tabColor = "00FF00"  # Green
                print(f"  🟢 Tab for sheet {sheet_name} colored GREEN (no DNC)")
...
def copy_entire_sheet(source_sheet, target_sheet):
    """Copy an entire sheet from source to target."""
    try:
...
def split_audit_data(input_file, output_dir='audit_leaders'):
    ...
    try:
        ...
        # Get all audit leaders from all sheets
        audit_leaders = get_all_audit_leaders(input_file)
        print(f"Found {len(audit_leaders)} audit leaders across all sheets")
        ...
        for leader in audit_leaders:
            ...
            if result:
                ...
                print(f"  No data found for {leader}, empty workbook created")
